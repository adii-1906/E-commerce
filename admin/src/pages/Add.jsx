...
const Add = ({token}) => {
    ...
    const [ subCategory, setSubCategory ] = useState("Topwwear");
            ...
            <div>
                ...
                <div className='flex gap-3'>
                    <div onClick={(e) => setSizes(prev => prev.includes("S") ? prev.filter(item => item !== "S") : [...prev, "S"])}>
                        <p className={`cursor-pointer px-3 py-2 ${sizes.includes("S") ? "bg-pink-100" : "bg-slate-200"}`}>S</p>
                    </div>
                    <div onClick={(e) => setSizes(prev => prev.includes("M") ? prev.filter(item => item !== "M") : [...prev, "M"])}>
                        <p className={`cursor-pointer px-3 py-2 ${sizes.includes("M") ? "bg-pink-100" : "bg-slate-200"}`}>M</p>
                    </div>
                    <div onClick={(e) => setSizes(prev => prev.includes("L") ? prev.filter(item => item !== "L") : [...prev, "L"])}>
                        <p className={`cursor-pointer px-3 py-2 ${sizes.includes("L") ? "bg-pink-100" : "bg-slate-200"}`}>L</p>
                    </div>
                    <div onClick={(e) => setSizes(prev => prev.includes("XL") ? prev.filter(item => item !== "XL") : [...prev, "XL"])}>
                        <p className={`cursor-pointer px-3 py-2 ${sizes.includes("XL") ? "bg-pink-100" : "bg-slate-200"}`}>XL</p>
                    </div>
                    <div onClick={(e) => setSizes(prev => prev.includes("XXL") ? prev.filter(item => item !== "XXL") : [...prev, "XXL"])}>
                        <p className={`cursor-pointer px-3 py-2 ${sizes.includes("XXL") ? "bg-pink-100" : "bg-slate-200"}`}>XXL</p>
                    </div>
                </div>
            </div>

            <div className='flex gap-2 mt-2'>
                <input onChange={(e) => setBestseller(prev => !prev)} checked={bestseller}
